...
This spec defines how to install `agent-pack/` into another codebase.

## Goal

Install a reusable `agent-pack` into a project without relying on public egress,
global config mutation, telemetry, public tunnels, or cookie import.

## Supported Install Modes

| Mode | Target | Use case |
|---|---|---|
| Repo-local | `<repo>/.agent/` | Default company-safe install. |
| Vendored source | `<repo>/tools/agent-pack/` | Repo owns a pinned copy. |
| Approved user directory | `<home>/.agent-pack/` | Individual opt-in install. |
| Generated host sidecar | `<repo>/.agents/skills/agent-pack/` or host equivalent | Host-specific generated skill artifacts. |
...
```text
agent-pack/policies/enterprise-default.json
```
...
9. Report agents, workflows, stacks, domains, and tool providers made available.
10. Report host workflow docs included with install.
...
```text
<target>/.agent/install-manifest.json
```
...
- Upgrade skill is `agent-pack-upgrade`, not `tstack-upgrade`.
- Agent-pack install exposes skills, agents, workflows, stacks, domains, and tool-provider metadata.
