...
Map `claude-code-best-practice/` content into `agent-pack/` without importing repo bloat.
...
Only first three belong in `agent-pack` directly.
...
- `adapt`: useful, but rewrite into existing `agent-pack` model
...
- `.claude/hooks/HOOKS-README.md`
  ...
  Why: `agent-pack/hooks/` already exists but docs are thin.
...
These should become `agent-pack` docs, skills, or workflow registry entries:
...
- imports repo weight without improving agent-pack package quality
...
- do not duplicate concepts already covered by `agent-pack`
