...
This map translates useful `agent-pack/test` coverage patterns into
`agent-pack/` tests.

The goal is not to copy every agent-pack test. The goal is to carry over the test
classes that protect this package's behavior: agent-pack-style skill templates,
safe defaults, installability in other repos, host portability, and automation
support.

## Coverage Status

| agent-pack test family | What it protects | agent-pack equivalent | Status |
|---|---|---|---|
...
| agent-pack test family | Reason to defer |
|---|---|
