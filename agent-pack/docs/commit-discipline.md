...
|---|---|
| `docs: define Agent Pack module map` | Adds one architecture decision document. |
...
- The diff is inside `agent-pack/` unless the task explicitly says
  otherwise.
- The commit does not modify upstream `agent-pack/`.
...
docs: inventory agent-pack migration decisions
```
