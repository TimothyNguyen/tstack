...
Roll back to a previous `agent-pack` version after a bad upgrade.
...
git commit -m "chore: rollback agent-pack to v$(cat .agent/VERSION)"
```
...
```bash
npm install --save-dev agent-pack@0.1.0
```
...
git commit -m "chore: rollback agent-pack to v0.1.0"
```
