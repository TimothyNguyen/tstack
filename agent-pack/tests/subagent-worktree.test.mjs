...
test('allocateSubagentWorktree creates isolated worktree under declared subagent directory', () => {
  ...
  assert.equal(allocation.branch, 'agent-pack/implementer-auth');
  assert.equal(
    path.relative(path.join(repo, '.agent-pack', 'subagents', 'implementer-auth'), allocation.worktreePath).startsWith('..'),
    false,
  );
  assert.equal(fs.existsSync(path.join(allocation.worktreePath, 'src', 'auth.js')), true);
  assert.equal(fs.existsSync(path.join(repo, '.agent-pack', 'subagents', 'implementer-auth', 'allocation.json')), true);
...
test('collectSubagentWorktreeResult records changed files from isolated worktree', () => {
  ...
  assert.equal(fs.existsSync(path.join(repo, '.agent-pack', 'subagents', 'test-engineer-auth', 'result.json')), true);
...
test('cleanupSubagentWorktree skips git-remove when worktree path does not exist', () => {
  const repo = makeRepo();
  const allocationDir = path.join(repo, '.agent-pack', 'subagents', 'implementer-phantom');
  fs.mkdirSync(allocationDir, { recursive: true });
  const phantomWorktree = path.join(repo, '.agent-pack', 'subagents', 'implementer-phantom', 'worktree');
  fs.writeFileSync(path.join(allocationDir, 'allocation.json'), JSON.stringify({
    id: 'implementer-phantom',
    role: 'implementer',
    branch: 'agent-pack/implementer-phantom',
    baseRef: 'HEAD',
