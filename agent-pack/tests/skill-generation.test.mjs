...
test('upgrade skill uses agent-pack name', () => {
  const skillsRoot = path.join(root, 'skills');
  assert.equal(fs.existsSync(path.join(skillsRoot, 'agent-pack-upgrade', 'SKILL.md.tmpl')), true);
  assert.equal(fs.existsSync(path.join(skillsRoot, 'agent-pack-upgrade', 'SKILL.md')), true);
...
test('install spec documents safe repo-local install contract', () => {
  ...
  assert.match(installSpec, /\.agent-pack/);
  ...
  assert.match(installSpec, /agent-pack-upgrade/);
});
