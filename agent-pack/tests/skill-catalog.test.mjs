...
const excludedSkills = new Set(['agent-pack-upgrade']);
...
function skillDirs() {
  ...
  return fs.readdirSync(skillsRoot, { withFileTypes: true })
    ...
    .filter((entry) => !excludedSkills.has(entry.name))
...
test('root router skill routing is consistent', () => {
  ...
  for (const skill of routedSkills) {
    if (!skill.startsWith('agent-pack-') && !skill.startsWith('adapter-') &&
...
test('catalog excludes agent-pack-only or disallowed defaults', () => {
  const catalog = read('docs/skill-catalog.md');

  assert.doesNotMatch(catalog, /plan-ceo-review/);
  assert.doesNotMatch(catalog, /agent-pack-upgrade/);
