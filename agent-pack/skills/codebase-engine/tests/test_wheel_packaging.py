...
`codebase-engine install` hard-exit with "not found in package" for real users.
...
def wheel_namelist(tmp_path_factory) -> set[str]:
    ...
    wheels = list(out.glob("codebase-engine-*.whl"))
...
def test_skill_artifact_ships_in_wheel(artifact: Path, wheel_namelist: set[str]) -> None:
    rel = "codebase-engine/" + artifact.relative_to(PKG).as_posix()
    assert rel in wheel_namelist, (
        f"{rel} is committed in the repo but NOT in the built wheel — "
        f"`codebase-engine install` would hard-exit for this host. Check the "
