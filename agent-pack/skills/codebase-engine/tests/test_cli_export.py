"""Integration tests for codebase-engine export subcommands and CLI commands.
...
def _make_graph(tmp_path: Path) -> Path:
    """Build a minimal graph.json + analysis/labels files in tmp_path/codebase-out/."""
    ...
    from codebase_engine.analyze import god_nodes, surprising_connections
    from codebase_engine.export import to_json
    ...
    (out / ".codebase_analysis.json").write_text(json.dumps(analysis))
    (out / ".codebase_labels.json").write_text(
...
def test_export_html_creates_file(tmp_path):
    ...
    html = tmp_path / "codebase-out" / "graph.html"
    assert html.exists()
...
# ── codebase-engine export neo4j (cypher) ───────────────────────────────────────────

def test_export_neo4j_creates_cypher(tmp_path):
    _make_graph(tmp_path)
    r = _run(["export", "neo4j"], tmp_path)
    assert r.returncode == 0, r.stderr
    cypher = tmp_path / "codebase-out" / "cypher.txt"
...
# ── codebase-engine export falkordb (cypher) ────────────────────────────────────────
...
def test_query_uses_codebase_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
    env["CODEBASE_OUT"] = custom_out.name
...
def test_path_uses_codebase_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
...
def test_explain_uses_codebase_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
...
# ── codebase-engine export unknown format ───────────────────────────────────────────
...
# Regression test for #934 - cluster-only crashes when codebase-out/ doesn't exist

def test_cluster_only_creates_output_dir_when_missing(tmp_path):
    """cluster-only must not crash with FileNotFoundError when codebase-out/ is absent (#934)."""
    # Build graph.json somewhere other than the default codebase-out/ location
    # so we can point --graph at it while codebase-out/ doesn't exist yet.
    ...
    assert r.returncode == 0, r.stderr
    assert (tmp_path / "codebase-out" / "GRAPH_REPORT.md").exists()
...
def test_cluster_only_remaps_labels_to_previous_cids(tmp_path):
    ...
    .codebase_labels.json keeps tracking the same conceptual communities after
    ...
    labels_json = out / ".codebase_labels.json"
...
# ── communities-fallback when .codebase_analysis.json is absent ──────────────
# The watch / post-commit rebuild path only writes graph.json + GRAPH_REPORT.md;
# it does NOT regenerate .codebase_analysis.json. The full `codebase-engine extract`
...
# Without these tests, `codebase-engine export html|obsidian|wiki|svg|graphml|neo4j`
...
def test_export_html_falls_back_to_node_community_attribute(tmp_path):
    """When .codebase_analysis.json is absent, export html should reconstruct
    ...
    (out / ".codebase_analysis.json").unlink()
...
def test_export_html_fallback_recovers_multiple_communities(tmp_path):
    ...
    analysis = json.loads((out / ".codebase_analysis.json").read_text(encoding="utf-8"))
