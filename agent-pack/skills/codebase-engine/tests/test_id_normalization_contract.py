...
Both callers now delegate to :mod:`codebase_engine.ids`, so they share one
...
from codebase_engine.build import _normalize_id
from codebase_engine.extract import _make_id
from codebase_engine.ids import make_id, normalize_id
...
def test_both_callers_share_one_implementation():
    ...
    underlying function object as codebase_engine.ids.normalize_id."""
    # build._normalize_id is imported directly from codebase_engine.ids.
    ...
    from codebase_engine.mcp_ingest import _make_id as _mcp_make_id
    from codebase_engine.symbol_resolution import _bash_make_id
