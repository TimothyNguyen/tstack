...
skillgen renders codebase-engine's committed skill artifacts from human-edited
...
def test_no_version_or_timestamp_in_output():
    ...
    from codebase_engine.__main__ import __version__
...
def _claude_artifacts():
    ...
    core = next(a for a in arts if a.path == "codebase_engine/skill.md")
    refs = {a.path.rsplit("/", 1)[-1]: a.content for a in arts if a.path != "codebase_engine/skill.md"}
...
def test_lean_core_has_no_reference_only_content():
    ...
    assert "from codebase_engine.build import build_merge" not in core
    assert "codebase-engine cluster-only ." not in core
    ...
    assert "codebase-engine export wiki" not in core
    assert "codebase-engine export neo4j" not in core
    # The add / watch / hook flows live only in their references.
    assert "from codebase_engine.ingest import ingest" not in core
    assert "codebase-engine hook install" not in core
    assert "python3 -m codebase_engine.watch" not in core
...
def test_lean_core_runs_default_pipeline_with_zero_references():
    ...
    for needed in (
        "### Step 1 - Ensure codebase-engine is installed",
        "### Step 2 - Detect files",
...
def test_references_contain_no_core_pipeline_content():
    ...
    core_only_markers = (
        "from codebase_engine.cluster import cluster, score_all",
        "### Step 4 - Build graph, cluster, analyze, generate outputs",
        "### Step 5 - Label communities",
        "## Honesty Rules",
    )
...
def test_query_heading_is_homed_in_core_stub_only():
    ...
    assert "## For /codebase-engine query" in core_headings
    assert "## For /codebase-engine query" not in query_headings
    # The deeper query content moved into the reference.
    assert "## For /codebase-engine path" in query_headings
    assert "## For /codebase-engine explain" in query_headings
    assert "## For /codebase-engine path" not in core_headings
...
UNIFIED_DESCRIPTION = (
    ...
    "or project content — especially when codebase-out/ exists, where the question "
    "should be treated as a codebase-engine query first. Turns any input (code, docs, "
...
def test_descriptions_are_unified():
    ...
    for key, p in platforms.items():
        ...
        assert "treat the question as a /codebase-engine query." not in body, f"[{key}] kept old wording"
        assert "clustered communities" not in body, f"[{key}] kept old wording"


def test_windows_frontmatter_name_and_shell_and_extra():
    """windows: codebase-engine-windows name, powershell install, troubleshooting tail."""
    core, _ = _platform_artifacts("windows")
    assert core.startswith("---\nname: codebase-engine-windows\n")
...
def test_every_platform_query_has_expansion_and_fallback():
    ...
    for key in ("claude", "codex", "windows", "opencode"):
        ...
        assert "## For /codebase-engine path" in q
        assert "## For /codebase-engine explain" in q
...
def test_monoliths_render_inline_single_file_no_references():
    ...
    for key in ("aider", "devin"):
        ...
        assert arts[0].path == f"codebase_engine/skill-{key}.md"
...
def test_monoliths_carry_the_1392_runbook_fixes():
    ...
    for key in ("aider", "devin"):
        ...
        assert ".codebase_cached.json').unlink(missing_ok=True)" in body
...
def test_always_on_renders_six_blocks():
    ...
    assert paths == [
        "codebase_engine/always_on/agents-md.md",
        "codebase_engine/always_on/antigravity-rules.md",
        "codebase_engine/always_on/claude-md.md",
        "codebase_engine/always_on/gemini-md.md",
        "codebase_engine/always_on/kiro-steering.md",
        "codebase_engine/always_on/vscode-instructions.md",
    ]
...
def test_always_on_included_in_full_render_not_per_platform():
    ...
    assert "codebase_engine/always_on/claude-md.md" in full
    assert "codebase_engine/always_on/claude-md.md" not in claude_only
...
def test_always_on_roundtrip_is_byte_faithful():
    ...
    codebase_engine.__main__, so the packaged markdown must round-trip exactly or those
...
def test_always_on_files_are_guarded_by_check(tmp_path):
    ...
    mutated = [
        ...
        if a.path == "codebase_engine/always_on/claude-md.md"
        else a
        for a in arts
    ]
...
def test_audit_reads_each_host_against_its_own_v8_body():
    ...
    """
    assert gen._v8_baseline_ref("claude") == "47042beb05d1f6dd2186c0c499ae2840ce604ead:codebase_engine/skill.md"
    assert gen._v8_baseline_ref("trae") == "47042beb05d1f6dd2186c0c499ae2840ce604ead:codebase_engine/skill-trae.md"
    assert gen._v8_baseline_ref("vscode") == "47042beb05d1f6dd2186c0c499ae2840ce604ead:codebase_engine/skill-vscode.md"
...
def test_trae_renders_native_agents_md_integration_not_claude():
    """trae wires `codebase-engine trae install` -> AGENTS.md, never `codebase-engine claude install`."""
    ...
    assert "codebase-engine trae install" in hooks
    assert "codebase-engine trae-cn install" in hooks
    assert "writes a `## codebase-engine` section to the local `AGENTS.md`" in hooks
    # The claude-flavored install command must NOT appear for trae.
    assert "codebase-engine claude install" not in hooks
    ...
    assert "wire codebase-engine into a project's AGENTS.md" in core
...
def test_trae_hooks_reference_includes_the_pretooluse_note():
    ...
    assert "Run `/codebase-engine --update` manually after code changes" in hooks
...
def test_claude_flavored_hosts_keep_their_hooks_text_unchanged():
    ...
    for key in ("claude", "droid", "codex", "windows", "kilo", "vscode"):
        core, refs = _platform_artifacts(key)
        hooks = refs["hooks.md"]
        assert "codebase-engine claude install" in hooks, f"[{key}] lost the claude install command"
...
def test_amp_renders_native_agents_md_integration_v8_faithfully():
    """amp wires `codebase-engine amp install` -> AGENTS.md exactly as its v8 body had it.
    ...
    assert "make codebase-engine always-on in Amp sessions" in hooks
    assert "instructs Amp to check the graph" in hooks
    # amp's single-line install/uninstall, no trae-cn alt comments.
    assert "codebase-engine amp install" in hooks
    assert "codebase-engine amp uninstall  # remove the section" in hooks
    assert "codebase-engine trae install" not in hooks
    assert "codebase-engine trae-cn" not in hooks
    assert "or: codebase-engine" not in hooks
...
def test_amp_audit_coverage_passes_against_its_own_v8():
    ...
    against a lean split. The audit reads origin/v8:codebase_engine/skill-amp.md and
    confirms every heading single-homes in amp's core + references.
    """
    platforms = gen.load_platforms()
    assert gen._v8_baseline_ref("amp") == "47042beb05d1f6dd2186c0c499ae2840ce604ead:codebase_engine/skill-amp.md"
