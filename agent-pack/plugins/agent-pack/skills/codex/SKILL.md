...
version: 0.1.1
description: |
  OpenAI Codex host adapter. Covers how to use this agent pack inside a
...
Use this adapter when this agent pack runs inside an OpenAI Codex agent
...
The generated agent-pack instructions for Codex live at:

```
.agent-pack/generated/codex/AGENTS.md
```
...
| `AGENT_PACK_ROOT` | Root of the installed agent pack |
...
```bash
node "${AGENT_PACK_ROOT}/adapters/seniorswe-concise/hooks/seniorswe-concise-activate.cjs"
```
...
echo '{"prompt":"'"$USER_PROMPT"'"}' | \
  node "${AGENT_PACK_ROOT}/adapters/seniorswe-concise/hooks/seniorswe-concise-mode-tracker.cjs"
```
...
1. Verify `AGENT_PACK_ROOT` points to the installed pack.
2. Read `AGENTS.md` to confirm agent-pack instructions are loaded.
