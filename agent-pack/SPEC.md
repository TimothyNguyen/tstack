...
## G
G1: Build `agent-pack`: reusable enterprise-safe authored definitions for skills, agents, workflows, stacks, domains, adapters, and tool providers that can be installed locally per project and adapted to Claude, Codex, Copilot, Strands/AgentCore, Google ADK, AG-UI, and future hosts.
...
C6: External agent-pack sources should be mined for patterns, not adopted wholesale where they conflict with enterprise policy.
...
C10: Agent-pack contents must explicitly exclude mobile/iOS workflows, public internet scraping, consumer browser automation, generic growth-hacking flows, and unrelated personal-productivity features.
C11: Skill directories must be easy to add, version, review, and install locally for company teams.
C12: Core folders must be project-agnostic. Project-specific stacks, domains, tools, and policies live in profiles or optional agent packs.
...
I8: Package source: `agent-pack/` contents, especially `hosts/`, `scripts/gen-skill-docs.mjs`, generated host artifacts, skill directories, agents, workflows, stacks, domains, adapters, and tool providers.
...
I13: Generic package root: `agent-pack/`.
...
V13: Generated skills include enterprise boundary text. Skills must tell agents not to read unrelated agent packs, secret stores, or global agent configs unless the active task requires it.
...
V19: No mobile carry-over. iOS QA, device tunnels, Swift templates, mobile debug bridge, and mobile-specific skills are excluded.
...
T1|x|Inventory agent-pack source candidates: host configs, generator, skill templates, docs, browser daemon, telemetry, update, pair-agent, cookie import|V1,V4,V14,I8
...
T16|x|Write migration guide for external source adoption into `agent-pack`, including keep/change/drop table|V14,I8
...
T27|x|Create top-level `agent-pack/` package boundary for authored skills, agents, workflows, stacks, domains, adapters, and tool providers|C12,I13,I14,V23,V24
