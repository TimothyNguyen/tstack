...
**Cause:** A path was constructed that traverses above `.architecture-agent/subagents/<id>/`.
...
```bash
rm -rf .architecture-agent/subagents/<id>/
```
...
1. Capture the manifest: `cat .architecture-agent/subagents/<id>/manifest.json`
2. Capture the partial result (if any): `cat .architecture-agent/subagents/<id>/result.json`
