...
Agent skill packs often require a "global install" step that mutates the developer's
...
- Runtime artifacts (subagent worktrees, audit logs) go to `.architecture-agent/`
  inside the consuming repo, not to the pack source tree.
- `doctor` validates only paths inside the target directory.
- The upgrade workflow (`architecture-agent-upgrade`) replaces target-directory files
  in place; it does not touch anything outside the target.

## Consequences

- Installing two different skill packs (or two versions) in the same machine never
