...
from codebase_engine.ids import make_id as _shared_make_id
from codebase_engine.security import sanitize_metadata
...
def _module_stem(module_name: str | None) -> str:
    """Return the final module component used to match codebase-engine source stems."""
...
def find_unique_python_symbol(
    ...
    """Resolve one imported symbol to exactly one codebase-engine node id."""
...
def resolve_cross_file_raw_calls(
    ...
    This intentionally preserves codebase-engine's existing behavior:
...
def resolve_bash_source_edges(
    ...
    ``bash_sources[].target_path`` contract (codebase-engine static-analysis policy):
        ...
        - Relative paths: resolved against the *source file's* directory
          ...
          codebase-engine, NOT bash runtime semantics. At runtime, ``source ./X``
    ...
    for result, path in zip(per_file, path_by_index):
        ...
        for source in bash_sources:
            ...
            # codebase-engine static-analysis policy (NOT bash runtime semantics;
