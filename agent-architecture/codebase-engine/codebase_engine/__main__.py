...
try:
    ...
    __version__ = _pkg_version("codebase-engine")
except Exception:
    __version__ = "unknown"
...
def __getattr__(name: str) -> str:
    ...
    # brick the CLI on `import codebase_engine.__main__` (#1121 follow-up).
...
def _default_graph_path() -> str:
    return str(Path(_CODEBASE_OUT) / "graph.json")
...
#
# Enterprise build note: always_on/ is absent from this build (host-injection is
# disabled per enterprise policy). _always_on() will raise RuntimeError if install
# is attempted, which is intentional — use the codebase-engine/SKILL.md agent skill
# instead of injecting always-on blocks into host config files.
...
def uninstall_all(project_dir: Path | None = None, purge: bool = False) -> None:
    ...
    # Git hook
    try:
        from codebase_engine.hooks import uninstall as hook_uninstall
    ...
    if cmd == "install":
        ...
        if subcmd == "list":
            ...
            if not base_url or not default_model or not env_key:
                ...
                sys.exit(1)
            from codebase_engine.llm import provider_base_url_ok
        ...
        print("error: 'prs' command not available in enterprise build.", file=sys.stderr)
        sys.exit(1)
    elif cmd == "hook":
        from codebase_engine.hooks import (
        ...
        if len(sys.argv) < 4:
            ...
            sys.exit(1)
        from codebase_engine.serve import _score_nodes
        from networkx.readwrite import json_graph
        import networkx as _nx
        ...
        if len(sys.argv) < 3:
            ...
            sys.exit(1)
        from codebase_engine.ingest import ingest as _ingest
        ...
        if not watch_path.exists():
            ...
            sys.exit(1)
        from codebase_engine.watch import watch as _watch
        ...
        from codebase_engine.cluster import cluster, score_all, remap_communities_to_previous
        from codebase_engine.analyze import (
            god_nodes,
        ...
        from codebase_engine.security import check_graph_file_size_cap as _check_cap
        _over_cap = False
        try:
        ...
        from codebase_engine.export import _git_head as _gh
        _commit = _gh()
        ...
        from codebase_engine.tree_html import write_tree_html, DEFAULT_MAX_CHILDREN
        ...
        import networkx as _nx
        from networkx.readwrite import json_graph as _jg
        from codebase_engine.build import prefix_graph_for_global as _prefix
        graphs = []
        ...
        while i < len(args):
            ...
            if a == "--graph" and i + 1 < len(args):
                ...
                if candidate.name == "graph.json" or candidate.suffix.lower() == ".json":
                    ...
                    graph_path = candidate / _CODEBASE_OUT / "graph.json"
                graph_path_explicit = True
                i += 1
            else:
                i += 1
        ...
        if subcmd == "callflow-html":
            from codebase_engine.callflow_html import write_callflow_html as _write_callflow_html
        ...
        from networkx.readwrite import json_graph as _jg
        from codebase_engine.build import build_from_json as _bfj
        from codebase_engine.security import check_graph_file_size_cap as _check_cap
        ...
        if subcmd == "html":
            from codebase_engine.export import to_html as _to_html
            if no_viz:
            ...
            from codebase_engine.export import to_obsidian as _to_obsidian, to_canvas as _to_canvas
            ...
            if not communities:
                ...
                sys.exit(1)
            print("error: 'wiki' export not available in enterprise build.", file=sys.stderr)
            sys.exit(1)

        elif subcmd == "svg":
            from codebase_engine.export import to_svg as _to_svg
            _to_svg(G, communities, str(out_dir / "graph.svg"),
                    community_labels=labels or None)
            print(f"graph.svg written - embeds in Obsidian, Notion, GitHub READMEs")

        elif subcmd == "graphml":
            from codebase_engine.export import to_graphml as _to_graphml
            _to_graphml(G, communities, str(out_dir / "graph.graphml"))
            print(f"graph.graphml written - open in Gephi, yEd, or any GraphML tool")

        elif subcmd == "neo4j":
            if push_uri:
                from codebase_engine.export import push_to_neo4j as _push
                ...
                from codebase_engine.export import to_cypher as _to_cypher
            ...
            if push_uri:
                from codebase_engine.export import push_to_falkordb as _push
                result = _push(G, uri=push_uri, user=push_user,
                               password=push_password, communities=communities)
        ...
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        from codebase_engine.global_graph import (
        ...
        from codebase_engine.llm import (
        ...
        if needs_llm:
            if backend is None:
                ...
                sys.exit(1)
            if backend == "ollama":
                from codebase_engine.llm import _validate_ollama_base_url
        ...
        from codebase_engine.cache import (
        ...
        from codebase_engine.build import (
            build as _build,
            build_from_json as _build_from_json,
            build_merge as _build_merge,
        )
        from codebase_engine.cluster import cluster as _cluster, score_all as _score_all
        from codebase_engine.export import to_json as _to_json
        from codebase_engine.analyze import god_nodes as _god_nodes, surprising_connections as _surprising
