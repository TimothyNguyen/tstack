...
from codebase_engine.security import safe_fetch, safe_fetch_text, validate_url
...
def ingest(url: str, target_dir: Path, author: str | None = None, contributor: str | None = None) -> Path:
    """
    Fetch a URL and save it into target_dir as a codebase-engine-ready file.
    ...
    try:
        ...
        if url_type == "youtube":
            from codebase_engine.transcribe import download_audio
...
def save_query_result(
    ...
    Files are stored in memory_dir (typically codebase-out/memory/) with YAML frontmatter
    that codebase-engine's extractor reads as node metadata. This closes the feedback loop:
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Fetch a URL into a codebase-engine /raw folder")
    ...
    print(f"Ready for codebase-engine: {out}")
