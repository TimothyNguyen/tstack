...
from codebase_engine.analyze import _node_community_map
from codebase_engine.build import edge_data
...
_BACKUP_ARTIFACTS = [
    "graph.json",
    "GRAPH_REPORT.md",
    ".codebase_labels.json",
    ".codebase_analysis.json",
    "manifest.json",
    ".codebase_semantic_marker",
    "cost.json",
]
...
def backup_if_protected(out_dir: Path) -> "Path | None":
    ...
    - .codebase_semantic_marker is present (graph cost real LLM tokens), or
    - .codebase_labels.json contains at least one non-default community label
    ...
    Set CODEBASE_ENGINE_NO_BACKUP=1 to disable.
    """
    if os.environ.get("CODEBASE_ENGINE_NO_BACKUP"):
        return None
    out = Path(out_dir)
    if not (out / "graph.json").exists():
        return None

    is_semantic = (out / ".codebase_semantic_marker").exists()
    is_curated = False
        ...
        for name in _BACKUP_ARTIFACTS:
            ...
            if src.exists():
                try:
                    ...
                    pass
        if copied:
            print(f"[codebase-engine] backed up {reason} graph ({copied} files) -> {backup_dir.name}/")
        return backup_dir
    except Exception as exc:
        import sys
        print(f"[codebase-engine] warning: backup failed ({exc}) - continuing with overwrite", file=sys.stderr)
        return None
...
def _strip_diacritics(text: str | None) -> str:
    """NFKD-decompose `text` and remove combining characters to get ASCII-safe base letters."""
    import unicodedata
...
def _yaml_str(s: str) -> str:
    ...
    See `codebase_engine.ingest._yaml_str` for the full rationale; duplicated here to
...
def _viz_node_limit() -> int:
    """Return the effective viz node limit, honoring CODEBASE_ENGINE_VIZ_NODE_LIMIT env var.
    ...
    """
    import os
    raw = os.environ.get("CODEBASE_ENGINE_VIZ_NODE_LIMIT")
...
def _html_styles() -> str:
    """Return the CSS <style> block for the interactive graph HTML viewer."""
    return """<style>
...
def _hyperedge_script(hyperedges_json: str) -> str:
    """Return a <script> block that renders hyperedges as shaded D3 regions."""
    return f"""<script>
...
def _html_script(nodes_json: str, edges_json: str, legend_json: str) -> str:
    """Return the main D3 force-graph <script> block with embedded node/edge JSON."""
    return f"""<script>
...
def to_json(G: nx.Graph, communities: dict[int, list[str]], output_path: str, *, force: bool = False, built_at_commit: str | None = None, community_labels: dict[int, str] | None = None) -> bool:
    """Serialise the graph + community metadata to graph.json in node-link format.

    Returns True if the file was written, False if skipped (existing file is larger
    than the new one and force=False, preventing silent graph shrinkage).
    """
        ...
        try:
            ...
            if new_n < existing_n:
                ...
                print(
                    ...
                    f"Run a full rebuild (/codebase-engine .) to be safe, or pass force=True "
...
def to_cypher(G: nx.Graph, output_path: str) -> None:
    """Export the graph as Neo4j Cypher CREATE statements to `output_path`."""
    lines = ["// Neo4j Cypher import - generated by /codebase-engine", ""]
...
def to_html(
    ...
    if G.number_of_nodes() > limit:
        ...
        raise ValueError(
            ...
            f"(limit: {limit}). Use --no-viz, raise CODEBASE_ENGINE_VIZ_NODE_LIMIT, "
    ...
    def _js_safe(obj) -> str:
        """JSON-serialise obj and escape </script> to prevent script-tag breakout."""
...
<head>
<meta charset="UTF-8">
<title>codebase-engine - {title}</title>
...
def to_obsidian(
    ...
    def safe_name(label: str) -> str:
        """Strip Obsidian-illegal chars and trailing .md extensions from a node label."""
    ...
    def _dominant_confidence(node_id: str) -> str:
        """Return the most frequent confidence value among all edges touching this node."""
        confs = []
    ...
    # Map file_type → codebase-engine tag
    _FTYPE_TAG = {
        "code": "codebase-engine/code",
        "document": "codebase-engine/document",
        "paper": "codebase-engine/paper",
        "image": "codebase-engine/image",
    }
    ...
    for node_id, data in G.nodes(data=True):
        ...
        ftype_tag = _FTYPE_TAG.get(ftype, f"codebase-engine/{ftype}" if ftype else "codebase-engine/document")
        dom_conf = _dominant_confidence(node_id)
        conf_tag = f"codebase-engine/{dom_conf}"
    ...
    def _community_reach(node_id: str) -> int:
        """Count how many distinct other communities this node has edges into."""
        neighbor_cids = {
            node_community[nb]
...
def to_canvas(
    ...
    def safe_name(label: str) -> str:
        """Strip canvas-illegal chars and .md suffixes from a node label for canvas filenames."""
        cleaned = re.sub(r'[\\/*?:"<>|#^[\]]', "", label.replace("\r\n", " ").replace("\r", " ").replace("\n", " ")).strip()
        cleaned = re.sub(r"\.(md|mdx|qmd|markdown)$", "", cleaned, flags=re.IGNORECASE)
        return _cap_filename(cleaned) if cleaned else "unnamed"
...
def push_to_neo4j(
    ...
    def _safe_rel(relation: str) -> str:
        """Sanitize a relation string to a Neo4j-safe relationship type (uppercase, underscores only)."""
...
def push_to_falkordb(
    ...
    communities: dict[int, list[str]] | None = None,
    graph_name: str = "codebase-engine",
) -> dict[str, int]:
    ...
    identical to push_to_neo4j. Differences from the Neo4j path:
      ...
      - a named graph is selected via db.select_graph(graph_name) (default
        "codebase-engine"); FalkorDB keys each graph by name in the same instance.
    ...
    def _safe_rel(relation: str) -> str:
        """Sanitize a relation string to a FalkorDB-safe relationship type (uppercase, underscores only)."""
        return re.sub(r"[^A-Z0-9_]", "_", relation.upper().replace(" ", "_").replace("-", "_")) or "RELATED_TO"
