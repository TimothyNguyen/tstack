...
from codebase_engine.serve import (
...
def test_score_nodes_multiword_exact_label_outranks_superset():
    ...
    Regression for the `codebase-engine path` "No path found" bug: every node sharing
...
def test_query_terms_filters_only_short_english_terms(monkeypatch):
    import codebase_engine.serve as serve_mod
...
def test_load_graph_missing_file(tmp_path):
    codebase_dir = tmp_path / "codebase-out"
    codebase_dir.mkdir()
    with pytest.raises(SystemExit):
        _load_graph(str(codebase_dir / "nonexistent.json"))
...
def test_query_graph_text_context_filter_aliases_resolve():
    import networkx as nx
    from codebase_engine.serve import _normalize_context_filters
