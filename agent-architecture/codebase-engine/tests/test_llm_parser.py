...
- The CODEBASE_ENGINE_CLAUDE_CLI_MODEL env-var passthrough
"""
...
def test_uses_system_prompt_not_append(mock_run, _which):
    ...
    layering codebase-engine's extraction prompt on top of Claude Code's
...
def test_model_env_var_adds_model_flag(mock_run, _which, monkeypatch):
    """CODEBASE_ENGINE_CLAUDE_CLI_MODEL must be forwarded to claude -p --model."""
    monkeypatch.setenv("CODEBASE_ENGINE_CLAUDE_CLI_MODEL", "haiku")
    mock_run.return_value.returncode = 0
    mock_run.return_value.stdout = _make_envelope({"nodes": [], "edges": [], "hyperedges": []})
    mock_run.return_value.stderr = ""
    llm._call_claude_cli("payload")
...
def test_no_model_flag_when_env_var_unset(mock_run, _which, monkeypatch):
    ...
    monkeypatch.delenv("CODEBASE_ENGINE_CLAUDE_CLI_MODEL", raising=False)
    mock_run.return_value.returncode = 0
    mock_run.return_value.stdout = _make_envelope({"nodes": [], "edges": [], "hyperedges": []})
    mock_run.return_value.stderr = ""
    llm._call_claude_cli("payload")
