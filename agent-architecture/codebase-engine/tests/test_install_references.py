...
The real claude bundle now ships in the package (codebase_engine/skills/claude/), so
...
PKG_DIR = Path(codebase_engine.__file__).parent
...
def test_single_version_stamp_covers_skill_and_references(tmp_path, fake_bundle):
    """One .codebase_version stamp versions SKILL.md + references/ together."""
    platform = fake_bundle
    _install(tmp_path, platform)
    skill_dir = tmp_path / ".claude" / "skills" / "codebase-engine"
    stamps = list(skill_dir.rglob(".codebase_version"))
    assert len(stamps) == 1
    assert stamps[0] == skill_dir / ".codebase_version"
...
def test_claude_install_ships_lean_core_and_references(tmp_path):
    ...
    assert (skill_dir / ".codebase_version").read_text() == mainmod.__version__
...
def test_gemini_install_references_all_resolve(tmp_path):
    ...
    skill = tmp_path / ".gemini" / "skills" / "codebase-engine" / "SKILL.md"
    assert skill.exists()
...
def test_pyproject_declares_references_globs():
    ...
    The references sidecar ships under codebase_engine/skills/<host>/references/ and the
    always-on injection blocks under codebase_engine/always_on/. The earlier
    skills/*/SKILL.md glob matched nothing (no codebase_engine/skills/<host>/SKILL.md
    exists; the skill bodies are codebase_engine/skill*.md, listed explicitly), so it was
    ...
    pkg_data = data["tool"]["setuptools"]["package-data"]["codebase-engine"]
...
def test_built_wheel_ships_the_full_skill_payload():
    ...
    assert "codebase_engine/skills/claude/references/extraction-spec.md" in names
    assert "codebase_engine/skills/trae/references/hooks.md" in names
    # amp is now a split host too; its bundle must ship like every other.
    assert "codebase_engine/skills/amp/references/hooks.md" in names
...
def test_monolith_install_clears_orphan_references(tmp_path, fake_bundle):
    ...
    skill_dst = tmp_path / ".aider" / "codebase-engine" / "SKILL.md"
...
def test_amp_user_install_carries_references(tmp_path, monkeypatch):
    ...
    amp's real bundle now ships in the package (codebase_engine/skills/amp/), so the
