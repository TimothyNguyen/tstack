"""Tests for codebase-engine/cache.py."""
import pytest
from pathlib import Path
from codebase_engine.cache import file_hash, cache_dir, load_cached, save_cached, cached_files, clear_cache, _body_content
...
def test_clear_cache(tmp_file, cache_root):
    """clear_cache removes all .json files from codebase-out/cache/ (all subdirs)."""
    save_cached(tmp_file, {"nodes": [], "edges": []}, root=cache_root)
    # Since v0.5.3 entries go into cache/ast/, not the flat cache/ dir
    cache_base = cache_root / "codebase-out" / "cache"
...
# so a committed ``codebase-out/cache/`` is portable across machines and
...
def test_save_cached_relativizes_source_file(tmp_path):
    ...
    import json
    from codebase_engine.cache import save_cached, file_hash, cache_dir
...
def test_load_cached_absolutizes_source_file(tmp_path):
    ...
    from codebase_engine.cache import save_cached, load_cached
...
def test_load_cached_passes_through_legacy_absolute_source_file(tmp_path):
    """Cache entries written by an older codebase-engine (with absolute source_file
    inside) must still load correctly: the absolutize step is a no-op for
    already-absolute values."""
    import json
    from codebase_engine.cache import load_cached, file_hash, cache_dir
...
def test_cache_portable_across_roots(tmp_path):
    ...
    import json
    import shutil
    from codebase_engine.cache import save_cached, load_cached, file_hash, cache_dir
...
# are only valid for the codebase-engine version that wrote them. Keying purely on
...
def test_ast_cache_invalidated_on_version_bump(tmp_path, monkeypatch):
    ...
    import codebase_engine.cache as cache_mod
    ...
    assert load_cached(f, root=tmp_path, kind="ast") is None, (
        "AST cache entry from a previous codebase-engine version must not be served"
    )
...
def test_legacy_unversioned_ast_entries_not_served(tmp_path):
    """Entries written by pre-versioning codebase-engine (flat cache/ or unversioned
    ...
    import json
    from codebase_engine.cache import file_hash, _CODEBASE_OUT
    ...
    unversioned = tmp_path / _CODEBASE_OUT / "cache" / "ast"
