...
from codebase_engine.diagnostics import (
    diagnose_extraction,
    diagnose_file,
...
def test_diagnose_file_reads_json_and_formats_report(tmp_path: Path) -> None:
    ...
    assert "[codebase-engine] MultiDiGraph edge-collapse diagnostic" in report
...
def test_diagnose_multigraph_cli_human_output(monkeypatch, tmp_path: Path, capsys) -> None:
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["codebase-engine", "diagnose", "multigraph", "--graph", str(graph_path)],
    )

    mainmod.main()

    out = capsys.readouterr().out
    assert "[codebase-engine] MultiDiGraph edge-collapse diagnostic" in out
...
def test_diagnose_multigraph_cli_undirected_override(monkeypatch, tmp_path: Path, capsys) -> None:
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["codebase-engine", "diagnose", "multigraph", "--graph", str(graph_path), "--undirected"],
    )
...
def test_diagnose_multigraph_cli_json_output(monkeypatch, tmp_path: Path, capsys) -> None:
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["codebase-engine", "diagnose", "multigraph", "--graph", str(graph_path), "--json"],
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["wrong"], "Usage: codebase-engine diagnose multigraph"),
...
def test_diagnose_multigraph_cli_usage_errors(
    ...
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(mainmod.sys, "argv", ["codebase-engine", "diagnose", *argv_tail])
