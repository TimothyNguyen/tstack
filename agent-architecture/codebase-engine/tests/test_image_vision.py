...
def test_pdf_routed_through_pypdf_not_readtext(tmp_path, monkeypatch):
    ...
    import codebase_engine.detect as detect
...
def test_capability_flags(monkeypatch):
    ...
    monkeypatch.delenv("CODEBASE_ENGINE_OLLAMA_VISION", raising=False)
    assert not llm._backend_supports_vision("ollama")
    monkeypatch.setenv("CODEBASE_ENGINE_OLLAMA_VISION", "1")
