...
from codebase_engine.watch import _notify_only, _WATCHED_EXTENSIONS, _rebuild_lock, _check_shrink
...
def test_notify_only_creates_flag(tmp_path):
    ...
    flag = tmp_path / "codebase-out" / "needs_update"
    assert flag.exists()
    assert flag.read_text() == "1"

def test_notify_only_creates_flag_dir(tmp_path):
    # codebase-out dir does not exist yet
    assert not (tmp_path / "codebase-out").exists()
    _notify_only(tmp_path)
    assert (tmp_path / "codebase-out").is_dir()
...
def test_check_update_with_flag_returns_true_and_prints(tmp_path, capsys):
    ...
    assert "codebase-engine --update" in out
...
def test_watch_raises_without_watchdog(tmp_path, monkeypatch):
    ...
    from codebase_engine.watch import watch
...
def test_codebase_root_preserves_relative_when_invoked_with_relative_path(tmp_path, monkeypatch):
    """#777: ``.codebase_root`` stores the user-supplied path (``.``), not the
    resolved absolute, so a committed ``codebase-out/.codebase_root`` is
    ...
    saved = (corpus / "codebase-out" / ".codebase_root").read_text(encoding="utf-8")
    assert saved == ".", (
        f".codebase_root must preserve the user-supplied path; got {saved!r}"
    )


def test_codebase_root_preserves_absolute_when_user_supplied(tmp_path):
    """When the caller supplies an absolute path, ``.codebase_root`` stores
...
def test_rebuild_code_evicts_nodes_from_deleted_files(tmp_path):
    """#1007: codebase-engine update (_rebuild_code with no changed_paths) must remove
    ...
    assert _rebuild_code(corpus, acquire_lock=False) is True
    graph_path = corpus / "codebase-out" / "graph.json"
...
def test_rebuild_code_evicts_removed_symbol_from_surviving_file(tmp_path):
    """#1116: codebase-engine update (_rebuild_code with no changed_paths) must prune a
...
def test_rebuild_code_preupgrade_marker_less_node_one_cycle_lag(tmp_path):
    ...
    markers. On the first `codebase-engine update` after upgrading, a symbol removed
...
def test_rebuild_code_is_idempotent_when_cluster_ids_flap(tmp_path, monkeypatch):
    ...
    graph_path = tmp_path / "codebase-out" / "graph.json"
    report_path = tmp_path / "codebase-out" / "GRAPH_REPORT.md"
...
# --- .codebaseignore honored in watch handler (gh-928) ---
...
def test_watch_handler_honors_codebaseignore(tmp_path, monkeypatch):
    """gh-928: the watch Handler must short-circuit paths matching
    .codebaseignore so busy volumes (node_modules churn, build artefacts,
    ...
    (watch_root / ".codebaseignore").write_text("node_modules/\nbuild/\n", encoding="utf-8")
...
@pytest.mark.skipif(not _watchdog_available(), reason="watchdog not installed")
def test_watch_loads_codebaseignore_once(tmp_path, monkeypatch):
    """gh-928: .codebaseignore must be parsed exactly once at watch() startup,
    ...
    calls = {"n": 0}
    real_loader = detect_mod._load_codebaseignore
    ...
    monkeypatch.setattr(watch_mod, "_load_codebaseignore", counting_loader)
    ...
    time.sleep(0.7)
    assert calls["n"] == 1, f"_load_codebaseignore called {calls['n']} times; expected 1"
...
def test_rebuild_code_accepts_repo_relative_changed_path_for_subdir_root(tmp_path):
    ...
    try:
        ...
        graph_path = src / "codebase-out" / "graph.json"
        before = json.loads(graph_path.read_text(encoding="utf-8"))
...
def test_queue_and_drain_pending_round_trip(tmp_path):
    ...
    from codebase_engine.watch import _queue_pending, _drain_pending, _PENDING_FILENAME
...
def test_drain_pending_dedupes_and_skips_blank_lines(tmp_path):
    ...
    from codebase_engine.watch import _queue_pending, _drain_pending
...
def test_queue_pending_noop_on_empty_list(tmp_path):
    ...
    from codebase_engine.watch import _queue_pending, _PENDING_FILENAME
...
def test_rebuild_code_queues_on_lock_contention(tmp_path, monkeypatch, capsys):
    ...
    from codebase_engine.watch import _rebuild_code, _rebuild_lock, _PENDING_FILENAME
...
def test_merge_changed_paths_dedupes_in_order():
    ...
    from codebase_engine.watch import _merge_changed_paths
