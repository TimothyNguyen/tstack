...
def test_install_contains_expected_rules(tmp_path):
    ...
    assert "codebase-engine update" in content
...
def test_uninstall_removes_section(tmp_path):
    """Removes the codebase-engine section after it was installed."""
    claude_install(tmp_path)
...
def test_uninstall_preserves_other_content(tmp_path):
    """Uninstall keeps pre-existing content outside the codebase-engine section."""
    target = tmp_path / "CLAUDE.md"
...
def test_uninstall_no_op_when_not_installed(tmp_path, capsys):
    """Uninstall on a CLAUDE.md without codebase-engine section prints a message and exits cleanly."""
    target = tmp_path / "CLAUDE.md"
...
def test_install_settings_json_idempotent(tmp_path):
    ...
    settings = json.loads(settings_path.read_text())
    hooks = settings.get("hooks", {}).get("PreToolUse", [])
    bash_hooks = [h for h in hooks if h.get("matcher") == "Bash" and "codebase-engine" in str(h)]
...
def test_uninstall_removes_settings_hook(tmp_path):
    ...
    if settings_path.exists():
        settings = json.loads(settings_path.read_text())
        hooks = settings.get("hooks", {}).get("PreToolUse", [])
        assert not any(h.get("matcher") == "Bash" and "codebase-engine" in str(h) for h in hooks)
