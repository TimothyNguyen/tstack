...
def test_app_builds_and_initialize_succeeds(tmp_path):
    ...
    with _client(app) as client:
        ...
        assert payload["result"]["serverInfo"]["name"] == "codebase-engine"
...
def test_api_key_bearer_ok(tmp_path):
    ...
    with _client(app) as client:
        ...
        assert resp.status_code == 200
        assert resp.json()["result"]["serverInfo"]["name"] == "codebase-engine"
...
def test_cli_defaults_to_stdio(monkeypatch):
    ...
    serve_mod._main(["codebase-out/graph.json"])
    assert calls.get("stdio") == "codebase-out/graph.json"
...
def test_cli_api_key_from_env(monkeypatch):
    captured = {}
    monkeypatch.setenv("CODEBASE_ENGINE_API_KEY", "from-env")
