...
Every platform codebase-engine knows about installs its SKILL.md at a specific real
...
def test_skill_roundtrip_at_real_destination(platform, project, tmp_path, monkeypatch):
    ...
    with patch("codebase-engine.__main__.Path.home", return_value=home):
        ...
        assert (dst.parent / ".codebase_version").read_text() == mainmod.__version__
        ...
        assert not (dst.parent / ".codebase_version").exists()
...
def test_vscode_install_uninstall_roundtrip(tmp_path, monkeypatch):
    """VS Code Copilot Chat round trip at ~/.copilot/skills/codebase-engine + instructions file."""
    home = tmp_path / "home"
    project_dir = tmp_path / "proj"
    home.mkdir()
    project_dir.mkdir()
    ...
    with patch("codebase-engine.__main__.Path.home", return_value=home):
        ...
        assert "## codebase-engine" in instructions.read_text(encoding="utf-8")
        assert (skill.parent / ".codebase_version").read_text() == mainmod.__version__
        ...
        # The codebase-engine section is stripped from the instructions file.
        if instructions.exists():
            assert "## codebase-engine" not in instructions.read_text(encoding="utf-8")
...
def test_install_entrypoint_roundtrip_for_progressive_and_monolith(tmp_path):
    ...
    """
    for platform, rel in (
        ("claude", Path(".claude") / "skills" / "codebase-engine"),
        ("aider", Path(".aider") / "codebase-engine"),
    ):
...
def test_monolith_to_progressive_upgrade(tmp_path, fake_progressive_bundle):
    ...
    (skill_dir / ".codebase_version").write_text("0.0.1", encoding="utf-8")
    assert not (skill_dir / "references").exists()
