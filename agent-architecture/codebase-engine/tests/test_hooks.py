...
import pytest
from codebase_engine.hooks import install, uninstall, status, _hooks_dir, _HOOK_MARKER, _CHECKOUT_MARKER
...
def test_hook_skips_head_on_exe():
    ...
    from codebase_engine.hooks import _PYTHON_DETECT
...
def test_install_embeds_pinned_interpreter(tmp_path):
    ...
    codebase-engine launcher on PATH (uv tool / pipx isolation, #1127).

    When codebase-engine is installed via `uv tool install codebase-engine` or `pipx install
    codebase-engine`, the interpreter lives in an isolated venv and the launcher is in
    ~/.local/bin.  GUI git clients and CI runners often run with a minimal PATH
    that omits that directory, so `command -v codebase-engine` fails, the python3/python
    fallbacks cannot import codebase_engine (wrong venv), and the hook silently exits 0.
...
def test_hook_check_no_additionalContext(tmp_path):
    """codebase-engine hook-check must not emit additionalContext — Codex Desktop rejects it."""
    import sys
    ...
    result = subprocess.run(
        [sys.executable, "-m", "codebase-engine", "hook-check"],
        cwd=tmp_path,
        capture_output=True,
        text=True,
    )
...
from codebase_engine.hooks import (  # noqa: E402
    _HOOK_SCRIPT,
    _CHECKOUT_SCRIPT,
...
def test_rebuild_bodies_read_codebase_root(name, body):
    """The rebuild must honour the persisted scan root rather than hardcoding the
    repo top (#1173). Both bodies read codebase-out/.codebase_root and pass the
    recovered root to _rebuild_code instead of the bare Path('.')."""
    assert "codebase-out/.codebase_root" in body, f"{name} ignores .codebase_root (#1173)"
...
def test_rebuild_bodies_with_codebase_root_are_valid_python():
    """The .codebase_root snippet must parse so a quoting slip can't ship a hook
    that crashes the moment git fires it (#1173)."""
    for body in (_REBUILD_BODY_COMMIT, _REBUILD_BODY_CHECKOUT):
        ast.parse(body)


def test_detached_launch_targets_codebase_python():
    """The launcher must run via the resolved $CODEBASE_ENGINE_PYTHON, not a bare
    `python`, so it uses the same interpreter the detection block selected."""
    snippet = _detached_launch(_REBUILD_BODY_COMMIT)
    assert snippet.startswith('"$CODEBASE_ENGINE_PYTHON" -c "')
