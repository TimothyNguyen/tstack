"""Tests for token-aware chunking and parallel chunk execution in codebase_engine.llm."""
import time
...
def test_pack_chunks_packs_small_files_together(tmp_path):
    ...
    from codebase_engine.llm import _pack_chunks_by_tokens
...
def test_corpus_parallel_runs_chunks_concurrently(tmp_path):
    ...
    from codebase_engine.llm import extract_corpus_parallel
...
def test_adaptive_retry_returns_directly_when_not_truncated(tmp_path):
    ...
    from codebase_engine.llm import _extract_with_adaptive_retry
