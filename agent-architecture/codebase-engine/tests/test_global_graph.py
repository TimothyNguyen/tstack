"""Tests for the global graph infrastructure (codebase_engine/global_graph.py),
prefix/prune helpers in codebase_engine/build.py, and the cross-repo guard in
codebase_engine/dedup.py."""
...
def test_prefix_graph_preserves_label():
    from codebase_engine.build import prefix_graph_for_global
...
def test_prune_repo_removes_correct_nodes():
    from codebase_engine.build import prune_repo_from_graph
    G = nx.Graph()
...
def test_merge_graphs_prefixes_ids(tmp_path):
    ...
    repo1 = tmp_path / "repo1" / "codebase-out"
    repo2 = tmp_path / "repo2" / "codebase-out"
