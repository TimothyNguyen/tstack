...
def test_codebase_engine_openai_model_wins_over_openai_model(monkeypatch):
    # model_env_key (CODEBASE_ENGINE_OPENAI_MODEL) is resolved at call time and takes
    # precedence over the import-time OPENAI_MODEL default.
    monkeypatch.setenv("OPENAI_MODEL", "env-default-model")
    monkeypatch.setenv("CODEBASE_ENGINE_OPENAI_MODEL", "codebase-engine-override-model")
    reloaded = importlib.reload(llm)
    try:
        assert reloaded._default_model_for_backend("openai") == "codebase-engine-override-model"
    finally:
        monkeypatch.undo()
        importlib.reload(llm)
