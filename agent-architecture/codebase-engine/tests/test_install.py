"""Tests for codebase-engine install --platform routing."""
import os
...
PLATFORMS = {
    "claude": (".claude/skills/codebase-engine/SKILL.md",),
    "codebuddy": (".codebuddy/skills/codebase-engine/SKILL.md",),
    "codex": (".codex/skills/codebase-engine/SKILL.md",),
    "opencode": (".config/opencode/skills/codebase-engine/SKILL.md",),
    "kilo": (
        ".config/kilo/skills/codebase-engine/SKILL.md",
        ".config/kilo/command/codebase-engine.md",
    ),
    "claw": (".openclaw/skills/codebase-engine/SKILL.md",),
    "droid": (".factory/skills/codebase-engine/SKILL.md",),
    "trae": (".trae/skills/codebase-engine/SKILL.md",),
    "trae-cn": (".trae-cn/skills/codebase-engine/SKILL.md",),
    "windows": (".claude/skills/codebase-engine/SKILL.md",),
}
...
def test_install_default_claude(tmp_path):
    ...
    assert (tmp_path / ".claude" / "skills" / "codebase-engine" / "SKILL.md").exists()


def test_install_codebuddy(tmp_path):
    _install(tmp_path, "codebuddy")
    assert (tmp_path / ".codebuddy" / "skills" / "codebase-engine" / "SKILL.md").exists()


def test_install_codex(tmp_path):
    _install(tmp_path, "codex")
    assert (tmp_path / ".codex" / "skills" / "codebase-engine" / "SKILL.md").exists()


def test_install_opencode(tmp_path):
    _install(tmp_path, "opencode")
    assert (
        tmp_path / ".config" / "opencode" / "skills" / "codebase-engine" / "SKILL.md"
    ).exists()
...
def test_install_project_claude_writes_project_scope(tmp_path, monkeypatch, capsys):
    ...
    assert not (home / ".claude" / "skills" / "codebase-engine" / "SKILL.md").exists()
    assert ".claude/skills/codebase-engine/SKILL.md" in (project / ".claude" / "CLAUDE.md").read_text()
    assert "~/.claude/skills/codebase-engine/SKILL.md" not in (project / ".claude" / "CLAUDE.md").read_text()
...
def test_install_project_codex_writes_skill_and_agents(tmp_path, monkeypatch):
    ...
    assert not (home / ".codex" / "skills" / "codebase-engine" / "SKILL.md").exists()
...
def test_claude_subcommand_project_install_and_uninstall_are_project_scoped(tmp_path, monkeypatch):
    ...
    project = tmp_path / "project"
    project.mkdir()
    user_skill = home / ".claude" / "skills" / "codebase-engine" / "SKILL.md"
    ...
    with patch("codebase-engine.__main__.Path.home", return_value=home):
        ...
        monkeypatch.setattr(sys, "argv", ["codebase-engine", "claude", "uninstall", "--project"])
        main()

    assert user_skill.exists()
    assert not (project / ".claude" / "skills" / "codebase-engine" / "SKILL.md").exists()
...
def test_codex_subcommand_project_install_and_uninstall_are_project_scoped(tmp_path, monkeypatch):
    ...
    project = tmp_path / "project"
    project.mkdir()
    user_skill = home / ".codex" / "skills" / "codebase-engine" / "SKILL.md"
    user_skill.parent.mkdir(parents=True)
    user_skill.write_text("user skill")
    monkeypatch.chdir(project)
    with patch("codebase-engine.__main__.Path.home", return_value=home):
        ...
        monkeypatch.setattr(sys, "argv", ["codebase-engine", "codex", "uninstall", "--project"])
        main()

    assert user_skill.exists()
    assert not (project / ".codex" / "skills" / "codebase-engine" / "SKILL.md").exists()
    assert not (project / "AGENTS.md").exists()
    hooks_path = project / ".codex" / "hooks.json"
    assert hooks_path.exists()
    assert "codebase-engine" not in hooks_path.read_text()
...
def test_install_claw(tmp_path):
    ...
    assert (tmp_path / ".openclaw" / "skills" / "codebase-engine" / "SKILL.md").exists()


def test_install_droid(tmp_path):
    _install(tmp_path, "droid")
    assert (tmp_path / ".factory" / "skills" / "codebase-engine" / "SKILL.md").exists()


def test_install_trae(tmp_path):
    _install(tmp_path, "trae")
    assert (tmp_path / ".trae" / "skills" / "codebase-engine" / "SKILL.md").exists()


def test_install_trae_cn(tmp_path):
    _install(tmp_path, "trae-cn")
    assert (tmp_path / ".trae-cn" / "skills" / "codebase-engine" / "SKILL.md").exists()
...
def test_codex_skill_contains_spawn_agent():
    ...
    skill = (Path(codebase_engine.__file__).parent / "skill-codex.md").read_text()
    assert "spawn_agent" in skill


def test_codex_skill_uses_codebase_engine_with_existing_graph():
    ...
    assert "skip Steps 1–5 entirely and jump straight to `## For /codebase-engine query`" in skill
    assert "codebase-engine query" in skill
    assert "codebase-engine explain" in skill
    assert "codebase-engine path" in skill


def test_codex_agents_install_mentions_dirty_graph_output(tmp_path):
    _agents_install(tmp_path, "codex")
    content = (tmp_path / "AGENTS.md").read_text()
    assert "Dirty codebase-out/ files are expected" in content
    assert "not a reason to skip codebase-engine" in content
...
def test_opencode_skill_contains_mention():
    ...
    skill = (Path(codebase_engine.__file__).parent / "skill-opencode.md").read_text()
...
def test_kilo_skill_mentions_task_tool():
    ...
    skill = (Path(codebase_engine.__file__).parent / "skill-kilo.md").read_text()
...
def test_claw_skill_uses_agent_tool_dispatch():
    ...
    skill = (Path(codebase_engine.__file__).parent / "skill-claw.md").read_text()
    b2 = skill[skill.index("**Step B2"):skill.index("**Step B3")]
...
def test_all_skill_files_exist_in_package():
    ...
    pkg = Path(codebase_engine.__file__).parent
    for name in (
        "skill.md",
        "skill-codex.md",
        "skill-opencode.md",
...
def test_codebuddy_install_writes_codebuddy_md(tmp_path):
    from codebase_engine.__main__ import codebuddy_install
    codebuddy_install(tmp_path)
    md = tmp_path / "CODEBUDDY.md"
    assert md.exists()
    assert "codebase-out/GRAPH_REPORT.md" in md.read_text()
...
def test_codebuddy_install_writes_hook(tmp_path):
    ...
    assert any("codebase-engine" in str(h) for h in hooks)
...
def test_codebuddy_install_idempotent(tmp_path):
    ...
    codebuddy_install(tmp_path)
    md = tmp_path / "CODEBUDDY.md"
    assert md.read_text().count("## codebase-engine") == 1
...
def test_codebuddy_install_merges_existing_codebuddy_md(tmp_path):
    ...
    assert "codebase-out/GRAPH_REPORT.md" in content


def test_codebuddy_uninstall_removes_section(tmp_path):
    from codebase_engine.__main__ import codebuddy_install, codebuddy_uninstall
    codebuddy_install(tmp_path)
...
def test_codebuddy_uninstall_removes_hook(tmp_path):
    ...
    if settings_path.exists():
        ...
        assert not any("codebase-engine" in str(h) for h in hooks)


def test_codebuddy_uninstall_noop_if_not_installed(tmp_path):
    from codebase_engine.__main__ import codebuddy_uninstall
...
def test_antigravity_uninstall_project_removes_project_skill_only(tmp_path, monkeypatch):
    ...
    assert not (project / ".agents" / "skills" / "codebase-engine" / "SKILL.md").exists()


def test_antigravity_global_install_writes_gemini_config_skills(tmp_path, monkeypatch):
    """Global `codebase-engine antigravity install` must write to ~/.gemini/config/skills/ (#1079)."""
    from codebase_engine.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    ...
    with patch("codebase-engine.__main__.Path.home", return_value=home):
        ...
        main()
    global_skill = home / ".gemini" / "config" / "skills" / "codebase-engine" / "SKILL.md"
    wrong_skill = home / ".agents" / "skills" / "codebase-engine" / "SKILL.md"
    ...
    assert (project / ".agents" / "rules" / "codebase-engine.md").exists()
    assert (project / ".agents" / "workflows" / "codebase-engine.md").exists()


def test_antigravity_global_uninstall_removes_gemini_config_skill(tmp_path, monkeypatch):
    """Global `codebase-engine antigravity uninstall` must remove from ~/.gemini/config/skills/ (#1079)."""
    from codebase_engine.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    ...
    with patch("codebase-engine.__main__.Path.home", return_value=home):
        ...
        monkeypatch.setattr(sys, "argv", ["codebase-engine", "antigravity", "uninstall"])
        main()
    assert not global_skill.exists(), f"skill not removed from {global_skill} after uninstall"
    # workspace files also cleaned up
    assert not (project / ".agents" / "rules" / "codebase-engine.md").exists()
    assert not (project / ".agents" / "workflows" / "codebase-engine.md").exists()
...
def _agents_install(tmp_path, platform):
    from codebase_engine.__main__ import _agents_install as _install_fn

    _install_fn(tmp_path, platform)


def _agents_uninstall(tmp_path, platform=""):
    from codebase_engine.__main__ import _agents_uninstall as _uninstall_fn
...
def test_codex_agents_install_writes_agents_md(tmp_path):
    ...
    assert "codebase-engine" in agents_md.read_text()
...
def test_agents_install_idempotent(tmp_path):
    ...
    _agents_install(tmp_path, "codex")
    content = (tmp_path / "AGENTS.md").read_text()
    assert content.count("## codebase-engine") == 1
...
def test_agents_install_appends_to_existing(tmp_path):
    ...
    assert "## codebase-engine" in content
...
def test_agents_uninstall_removes_section(tmp_path):
    ...
    agents_md = tmp_path / "AGENTS.md"
    # File deleted when it only contained codebase-engine section
...
def test_agents_uninstall_preserves_other_content(tmp_path):
    ...
    content = agents_md.read_text()
    assert "Do not break things." in content
    assert "## codebase-engine" not in content
...
def test_opencode_agents_install_writes_plugin(tmp_path):
    """opencode install writes .opencode/plugins/codebase-engine.js."""
    _agents_install(tmp_path, "opencode")
    plugin = tmp_path / ".opencode" / "plugins" / "codebase-engine.js"
...
def test_opencode_agents_install_registers_plugin_in_config(tmp_path):
    ...
    assert any("codebase-engine.js" in p for p in config.get("plugin", []))
...
def test_opencode_agents_install_merges_existing_config(tmp_path):
    ...
    assert any("codebase-engine.js" in p for p in config["plugin"])
...
def test_opencode_agents_uninstall_removes_plugin(tmp_path):
    ...
    if config_file.exists():
        config = _json.loads(config_file.read_text())
        assert not any("codebase-engine.js" in p for p in config.get("plugin", []))
...
def test_kilo_agents_install_writes_plugin(tmp_path):
    _agents_install(tmp_path, "kilo")
    plugin = tmp_path / ".kilo" / "plugins" / "codebase-engine.js"
    assert plugin.exists()
    assert "tool.execute.before" in plugin.read_text()
...
def test_kilo_agents_install_registers_plugin_in_config(tmp_path):
    ...
    config = _json.loads(config_file.read_text())
    assert (
        tmp_path / ".kilo" / "plugins" / "codebase-engine.js"
...
def test_kilo_agents_install_idempotent(tmp_path):
    ...
    plugin_uri = (tmp_path / ".kilo" / "plugins" / "codebase-engine.js").resolve().as_uri()
    assert content.count("## codebase-engine") == 1
...
def test_kilo_install_writes_global_and_project_artifacts(tmp_path):
    ...
    assert (home_dir / ".config" / "kilo" / "skills" / "codebase-engine" / "SKILL.md").exists()
    assert (home_dir / ".config" / "kilo" / "command" / "codebase-engine.md").exists()
    assert (project_dir / "AGENTS.md").exists()
    assert (project_dir / ".kilo" / "plugins" / "codebase-engine.js").exists()
...
def test_kilo_uninstall_removes_plugin_registration_and_command(tmp_path):
    ...
    assert not (home_dir / ".config" / "kilo" / "command" / "codebase-engine.md").exists()
    assert not (
        home_dir / ".config" / "kilo" / "skills" / "codebase-engine" / "SKILL.md"
    ).exists()
    assert not (project_dir / ".kilo" / "plugins" / "codebase-engine.js").exists()
    config_file = project_dir / ".kilo" / "kilo.json"
    if config_file.exists():
        config = _json.loads(config_file.read_text())
        assert (
            project_dir / ".kilo" / "plugins" / "codebase-engine.js"
        ).resolve().as_uri() not in config.get("plugin", [])
...
def test_cursor_install_writes_rule(tmp_path):
    """cursor install writes .cursor/rules/codebase-engine.mdc."""
    from codebase_engine.__main__ import _cursor_install

    _cursor_install(tmp_path)
    rule = tmp_path / ".cursor" / "rules" / "codebase-engine.mdc"
    assert rule.exists()
...
def test_cursor_uninstall_removes_rule(tmp_path):
    ...
    from codebase_engine.__main__ import _cursor_install, _cursor_uninstall
...
def test_cursor_uninstall_noop_if_not_installed(tmp_path):
    ...
    from codebase_engine.__main__ import _cursor_uninstall
...
def test_gemini_install_writes_gemini_md(tmp_path):
    from codebase_engine.__main__ import gemini_install
...
def test_gemini_uninstall_removes_section(tmp_path):
    from codebase_engine.__main__ import gemini_install, gemini_uninstall
...
def test_gemini_uninstall_noop_if_not_installed(tmp_path):
    from codebase_engine.__main__ import gemini_uninstall

    gemini_uninstall(tmp_path)  # should not raise


def test_amp_user_install_lands_in_config_agents(tmp_path, monkeypatch):
    """`codebase-engine amp install` (user scope) must drop the skill into an Amp search
    ...
    old = home / ".amp" / "skills" / "codebase-engine" / "SKILL.md"
...
def test_amp_install_cleans_legacy_amp_skills_dir(tmp_path, monkeypatch):
    """A pre-fix ~/.amp/skills/codebase-engine install is removed on the next install."""
    from codebase_engine.__main__ import main

    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    legacy = home / ".amp" / "skills" / "codebase-engine"
    ...
    assert (home / ".config" / "agents" / "skills" / "codebase-engine" / "SKILL.md").exists()


def test_amp_user_uninstall_removes_skill_and_agents(tmp_path, monkeypatch):
    """`codebase-engine amp uninstall` removes the user-scope skill and AGENTS.md section."""
    from codebase_engine.__main__ import main
    ...
    with patch("codebase-engine.__main__.Path.home", return_value=home):
        ...
        monkeypatch.setattr(sys, "argv", ["codebase-engine", "amp", "uninstall"])
        main()
...
def test_amp_project_install_lands_in_dot_agents(tmp_path, monkeypatch):
    ...
    assert (project / ".agents" / "skills" / "codebase-engine" / "SKILL.md").exists()
    assert not (project / ".amp" / "skills" / "codebase-engine" / "SKILL.md").exists()
    assert (project / "AGENTS.md").exists()
    # User scope untouched.
    assert not (home / ".config" / "agents" / "skills" / "codebase-engine" / "SKILL.md").exists()


def test_uninstall_all_removes_amp_user_skill(tmp_path, monkeypatch):
    """The user-scope `codebase-engine uninstall` enumeration removes the amp skill."""
    from codebase_engine.__main__ import main
