"""Tests for codebase-engine codebuddy install / uninstall commands."""
import json
...
def _skill_path_user(tmp_path):
    return tmp_path / ".codebuddy" / "skills" / "codebase-engine" / "SKILL.md"


def _skill_path_project(project_dir):
    return project_dir / ".codebuddy" / "skills" / "codebase-engine" / "SKILL.md"
...
# ---------------------------------------------------------------------------
# User-scope install (codebase-engine install --platform codebuddy)
# ---------------------------------------------------------------------------

def test_codebuddy_install_user_creates_skill_file(tmp_path):
    """User-scope install copies skill to ~/.codebuddy/skills/codebase-engine/SKILL.md."""
...
def test_codebuddy_skill_file_contains_frontmatter(tmp_path):
    """Installed skill file must include codebase-engine YAML frontmatter."""
    _codebuddy_install_user(tmp_path)
...
def test_codebuddy_skill_file_references_codebase_engine_query(tmp_path):
...
# ---------------------------------------------------------------------------
# Project-scope install (codebase-engine codebuddy install)
# ---------------------------------------------------------------------------

def test_codebuddy_install_project_writes_codebuddy_md(tmp_path):
    """Project-scope install writes CODEBUDDY.md with codebase-engine section."""
    ...
    assert "codebase-out/" in content
...
def test_codebuddy_install_hook_has_bash_matcher(tmp_path):
    ...
    assert any("codebase-engine" in str(h) for h in bash_hooks)
...
def test_codebuddy_install_hook_has_read_glob_matcher(tmp_path):
    ...
    assert any("codebase-engine" in str(h) for h in read_hooks)


def test_codebuddy_install_idempotent(tmp_path):
    """Re-install does not duplicate ## codebase-engine sections."""
    from codebase_engine.__main__ import codebuddy_install
    codebuddy_install(tmp_path)
    codebuddy_install(tmp_path)
    md = _codebuddy_md_path(tmp_path)
...
def test_codebuddy_install_upgrades_stale_section(tmp_path):
    """Re-install replaces an old codebase-engine section with the current template."""
    from codebase_engine.__main__ import codebuddy_install, _CODEBUDDY_MD_MARKER
...
def test_codebuddy_uninstall_removes_section(tmp_path):
    """Uninstall removes the ## codebase-engine section from CODEBUDDY.md."""
...
def test_codebuddy_uninstall_noop_if_no_section(tmp_path):
    """Uninstall should not error when CODEBUDDY.md exists but no codebase-engine section."""
    from codebase_engine.__main__ import codebuddy_uninstall
...
def test_codebuddy_uninstall_preserves_other_content(tmp_path):
    """Uninstall preserves non-codebase-engine content in CODEBUDDY.md."""
    from codebase_engine.__main__ import codebuddy_install, codebuddy_uninstall
    _codebuddy_md_path(tmp_path).write_text("# My project rules\n")
    codebuddy_install(tmp_path)
    codebuddy_uninstall(tmp_path)
    # When codebase-engine section was appended, uninstall removes it and the file
...
def test_uninstall_all_removes_codebuddy_md(tmp_path, monkeypatch):
    """codebase-engine uninstall must clean up CODEBUDDY.md."""
    from codebase_engine.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
...
def test_uninstall_all_removes_codebuddy_hook(tmp_path, monkeypatch):
    """codebase-engine uninstall must clean up .codebuddy/settings.json hooks."""
    from codebase_engine.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
...
def test_codebuddy_platform_skill_destination_project_scope(tmp_path):
    """Project-scope destination must be <project>/.codebuddy/skills/codebase-engine/SKILL.md."""
    from codebase_engine.__main__ import _platform_skill_destination
...
def test_codebuddy_in_main_help_text(capsys, monkeypatch):
    """`codebase-engine --help` must list codebuddy in the platform list and per-platform section."""
    from codebase_engine.__main__ import main
    ...
    assert "|codebuddy)" in captured or "codebuddy" in captured, (
        "codebuddy missing from `codebase-engine --help` platform list"
    )
...
def test_codebuddy_skill_file_exists_in_package():
    ...
    skill = Path(codebase_engine.__file__).parent / "skill.md"
    assert skill.exists(), "skill.md missing from package"


def test_codebuddy_installation_roundtrip(tmp_path):
    """Install then uninstall leaves no trace of codebase-engine CODEBUDDY.md or hook."""
    from codebase_engine.__main__ import codebuddy_install, codebuddy_uninstall
    # Pre-existing project file
    _codebuddy_md_path(tmp_path).write_text("# My project\n")
    # One section above codebase-engine to test cleanup
    codebuddy_install(tmp_path)
    codebuddy_uninstall(tmp_path)
