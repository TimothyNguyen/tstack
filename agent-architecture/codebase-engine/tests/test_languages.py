...
import pytest
from codebase_engine.extract import (
...
def test_kotlin_emits_in_file_calls():
    ...
    `identifier` rename — see codebase-engine-kmp's PythonParityTest."""
    r = extract_kotlin(FIXTURES / "sample.kt")
    calls = _calls(r)
...
def test_swift_extension_across_files_merges_into_canonical_type():
    ...
    from codebase_engine.extract import extract
    paths = sorted((FIXTURES / "swift_cross_file").glob("*.swift"))
    r = extract(paths, cache_root=Path("/tmp/codebase-engine-test-no-cache"))
...
from codebase_engine.extract import extract_elixir
...
# ── Objective-C ──────────────────────────────────────────────────────────────
from codebase_engine.extract import extract_objc
...
from codebase_engine.extract import extract_markdown
