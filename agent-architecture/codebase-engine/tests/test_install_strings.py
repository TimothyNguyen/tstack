...
These strings live in codebase_engine/__main__.py and are written into project-local
files (CLAUDE.md, AGENTS.md, GEMINI.md, .cursor/rules/, .kiro/steering/, etc.)
or into in-process hook payloads. Earlier versions of codebase-engine told every
assistant to "ALWAYS read codebase-out/GRAPH_REPORT.md before answering" —
...
def test_every_install_surface_recommends_codebase_engine_query():
    """All ten install surfaces must point the assistant at `codebase-engine query`
    ...
    for name, text in _INSTALL_TEXTS.items():
        if "codebase-engine query" not in text:
            missing.append(name)
    assert not missing, (
        f"these install surfaces no longer mention `codebase-engine query`: {missing}. "
...
def test_agents_section_does_not_skip_dirty_graph_output():
    assert "Dirty codebase-out/ files are expected" in _AGENTS_MD_SECTION
    assert "not a reason to skip codebase-engine" in _AGENTS_MD_SECTION
