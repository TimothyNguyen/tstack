...
function resolveDeclaredDir(envName, fallback) {
  ...
  if (!contained(ROOT, dir) && !process.env.PONYTAIL_ALLOW_EXTERNAL_STATE) {
...
function getConfigDir() {
  return resolveDeclaredDir('PONYTAIL_CONFIG_DIR', path.join('.architecture-agent', 'ponytail'));
}
...
function getStateDir() {
  ...
  return resolveDeclaredDir('PONYTAIL_STATE_DIR', path.join('.architecture-agent', 'state', 'ponytail'));
}
