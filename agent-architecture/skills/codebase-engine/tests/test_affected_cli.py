...
def test_affected_cli_reverse_traverses_impact_edges(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["codebase-engine", "affected", "Foo", "--graph", str(graph_path)],
    )
...
def test_affected_cli_relation_filter_limits_reverse_traversal(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["codebase-engine", "affected", "Foo", "--relation", "calls", "--graph", str(graph_path)],
    )
...
def test_affected_cli_forces_directed_on_undirected_graph(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["codebase-engine", "affected", "B", "--relation", "calls", "--graph", str(graph_path)],
    )
...
def test_affected_cli_loads_edges_keyed_graph(monkeypatch, tmp_path, capsys):
    """codebase-engine's `extract` writes graph.json with an "edges" key (not networkx's
    ...
    # Emulate codebase-engine extract output: top-level "edges" key instead of "links".
    data = json_graph.node_link_data(graph, edges="links")
...
def test_resolve_seed_bare_name_matches_callable_label():
    from codebase_engine.affected import resolve_seed
