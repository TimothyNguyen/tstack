...
const ARCHITECTURE_AGENT_GITIGNORE = [
...
function writeArchitectureAgentGitignore() {
  const dir = path.join(process.cwd(), '.architecture-agent');
  const file = path.join(dir, '.gitignore');
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(file, ARCHITECTURE_AGENT_GITIGNORE, 'utf8');
}
...
function install() {
  ...
  if (!DRY_RUN) {
    ...
    writeArchitectureAgentGitignore();
  }
