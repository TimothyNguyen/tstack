...
unsure. Off only: "normal mode". Default: **full**.
Switch: `/seniorswe-concise lite|full|ultra`.
...
- Mark deliberate simplifications with a `seniorswe-concise:` comment so shortcuts are visible, not ignorance. Name the ceiling and upgrade path: `# seniorswe-concise: global lock, upgrade to per-account locks if throughput matters`.
