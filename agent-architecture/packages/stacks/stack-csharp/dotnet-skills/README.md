...
skill pack so you can use the .NET team's curated agent skills without
...
|---|---|
| Claude Code | `~/.claude/skills/` (symlink each plugin's `skills/<name>` folder) or use the project's `.architecture-agent/` install target. |
...
See the **Vendored .NET Skill Packs** table in
