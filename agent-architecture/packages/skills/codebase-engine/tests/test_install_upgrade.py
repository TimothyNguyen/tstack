...
Pre-fix, the installers wrote a "## codebase-engine" section with report-first
instructions and skipped writing if the marker was already present. So users
who installed codebase-engine and then upgraded to the fixed package still had the
...
import codebase_engine.__main__ as mainmod


# A representative slice of the pre-fix text. Each platform's old install
# wrote a variant of "ALWAYS read codebase-out/GRAPH_REPORT.md before ...".
_OLD_CLAUDE_SECTION = """\
## codebase-engine

This project has a knowledge graph at codebase-out/ with god nodes, community structure, and cross-file relationships.

Rules:
- ALWAYS read codebase-out/GRAPH_REPORT.md before reading any source files, running grep/glob searches, or answering codebase questions. The graph is your primary map of the codebase.
- IF codebase-out/wiki/index.md EXISTS, navigate it instead of reading raw files
- For cross-module "how does X relate to Y" questions, prefer `codebase-engine query "<question>"`, `codebase-engine path "<A>" "<B>"`, or `codebase-engine explain "<concept>"` over grep — these traverse the graph's EXTRACTED + INFERRED edges instead of scanning files
- After modifying code, run `codebase-engine update .` to keep the graph current (AST-only, no API cost).
"""
...
code, your **first tool call must be** to read `codebase-out/GRAPH_REPORT.md` (if it exists).
...
- Before answering architecture or codebase questions, read codebase-out/GRAPH_REPORT.md for god nodes and community structure
- If codebase-out/wiki/index.md exists, navigate it instead of reading raw files
"""
...
codebase-engine: A knowledge graph of this project lives in `codebase-out/`. \
If `codebase-out/GRAPH_REPORT.md` exists, read it before answering architecture questions, \
...
_OLD_HOOK_PAYLOAD_SNIPPET = "Read codebase-out/GRAPH_REPORT.md for god nodes and community structure before searching raw files"


def _assert_no_report_first(text: str, ctx: str) -> None:
    assert "ALWAYS read codebase-out/GRAPH_REPORT.md" not in text, (
...
def _assert_query_first(text: str, ctx: str) -> None:
    assert "codebase-engine query" in text, (
        f"{ctx}: new 'codebase-engine query' guidance missing after upgrade"
    )
...
def test_claude_install_upgrades_stale_section(tmp_path, monkeypatch):
    ...
    `codebase-engine claude install` again after upgrading to a fixed package."""
    ...
    # Pre-existing non-codebase-engine content must be preserved
...
def test_claude_install_upgrades_stale_hook_payload(tmp_path, monkeypatch):
    ...
    assert "codebase-engine query" in new_settings_text, (
        "new hook payload should route to `codebase-engine query`"
    )
...
def test_cursor_install_upgrades_stale_rule(tmp_path, monkeypatch):
    """Same upgrade behavior for .cursor/rules/codebase-engine.mdc.
    The Cursor rule file is wholly codebase-engine-owned; overwrite on upgrade."""
    monkeypatch.chdir(tmp_path)
    rule_path = tmp_path / ".cursor" / "rules" / "codebase-engine.mdc"
    ...
    assert "read codebase-out/GRAPH_REPORT.md for god nodes and community structure" not in after
    _assert_query_first(after, ".cursor/rules/codebase-engine.mdc")
...
def test_kiro_install_upgrades_stale_steering(tmp_path, monkeypatch):
    """Same upgrade behavior for .kiro/steering/codebase-engine.md (wholly owned)."""
    monkeypatch.chdir(tmp_path)
    steering = tmp_path / ".kiro" / "steering" / "codebase-engine.md"
    ...
    _assert_query_first(after, ".kiro/steering/codebase-engine.md")
...
def test_kiro_install_ships_references_sidecar_and_version_stamp(tmp_path, monkeypatch):
    ...
    and .codebase_version stamp are written alongside SKILL.md (#1142).
    ...
    skill_dir = tmp_path / ".kiro" / "skills" / "codebase-engine"
    ...
    # .codebase_version stamp written
    version_file = skill_dir / ".codebase_version"
    assert version_file.exists(), ".codebase_version stamp must be written (#1142)"
    ...
    assert (tmp_path / ".kiro" / "steering" / "codebase-engine.md").exists()
    ...
    assert not (tmp_path / ".kiro" / "steering" / "codebase-engine.md").exists()
