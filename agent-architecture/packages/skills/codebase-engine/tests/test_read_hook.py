...
a source/doc file outside codebase-out/ when a graph exists, and otherwise stays
silent and fails open.
"""
import json
import subprocess

from codebase_engine.__main__ import _READ_SETTINGS_HOOK
...
def _run(tool_input, cwd, *, graph: bool):
    if graph:
        (cwd / "codebase-out").mkdir(parents=True, exist_ok=True)
        (cwd / "codebase-out" / "graph.json").write_text("{}", encoding="utf-8")
...
def test_nudges_on_source_read_with_graph(tmp_path):
    ...
    assert "codebase-engine query" in out
...
def test_nudge_payload_is_valid_pretooluse_json(tmp_path):
    ...
    assert "codebase-engine query" in payload["hookSpecificOutput"]["additionalContext"]


def test_silent_on_codebase_out_targets(tmp_path):
    """Reading the graph's own report must not start a go-read-the-graph loop."""
    out = _run({"file_path": "codebase-out/GRAPH_REPORT.md"}, tmp_path, graph=True).stdout
    assert out.strip() == ""
...
def test_fails_open_on_malformed_stdin(tmp_path):
    (tmp_path / "codebase-out").mkdir()
    (tmp_path / "codebase-out" / "graph.json").write_text("{}", encoding="utf-8")
    r = subprocess.run(
