...
const validator = path.join(toolProvidersRoot, 'atlassian-docs', 'scripts', 'validate_mcp_atlassian_profile.py');
const safeProfile = path.join(toolProvidersRoot, 'atlassian-docs', 'references', 'safe-profile.env.example');
...
test('atlassian-docs skill documents no codebase-engine vendoring and attachment denial', () => {
  const skill = fs.readFileSync(path.join(toolProvidersRoot, 'atlassian-docs', 'SKILL.md.tmpl'), 'utf8');
  const safety = fs.readFileSync(path.join(toolProvidersRoot, 'atlassian-docs', 'references', 'mcp-atlassian-safety.md'), 'utf8');
